...
import { ArrowLeft, Video, Lock, Play, Clock, FileText, BookOpen, ClipboardCheck, ChevronRight } from 'lucide-react';
...
const LessonCard = memo(({ lesson, index, onWatch }: { lesson: Lesson; index: number; onWatch: (l: Lesson) => void }) => {
  ...
  const type = lesson.lecture_type || "VIDEO";
  const isVideo = type === "VIDEO";
  const isTest = type === "TEST";
  const isDriveOrPdf = /drive\.google\.com/.test(lesson.video_url) || /\.pdf($|\?)/i.test(lesson.video_url);
    ...
    <Card className="cursor-pointer hover:shadow-lg transition-shadow" onClick={() => onWatch(lesson)}>
      <CardHeader className="p-0">
        <div className="aspect-video bg-muted rounded-t-lg relative overflow-hidden">
          {isVideo && extractYouTubeId(lesson.video_url) && (
            <img
              src={`https://img.youtube.com/vi/${extractYouTubeId(lesson.video_url)}/mqdefault.jpg`}
              alt={lesson.title}
...
const CoursePage = () => {
  ...
  const filteredLessons = lessons.filter(l => {
    const type = l.lecture_type || "VIDEO";
