...
const Dashboard = () => {
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        ...
        const [profileRes, enrollmentRes, progressRes] = await Promise.all([
          supabase.from('profiles').select('*').eq('id', user.id).single(),
          supabase.from('enrollments').select('course_id, courses ( * )').eq('user_id', user.id).eq('status', 'active'),
          supabase.from('user_progress').select('completed, lesson_id').eq('user_id', user.id),
        ]);
        ...
        if (!enrollmentRes.error && enrollmentRes.data) {
          const formattedCourses = enrollmentRes.data.map((item: any) => item.courses);
          setMyCourses(formattedCourses);
        }

        if (progressRes.data && progressRes.data.length > 0) {
          const completed = progressRes.data.filter((p: any) => p.completed).length;
  ...
  return (
    <div className="min-h-screen bg-muted/30 flex flex-col font-sans">
          ...
          <div className="space-y-6">
            ...
            {myCourses.length > 0 ? (
              <Card
                ...
                onClick={() => navigate(`/lesson/${myCourses[0].id}`)}
              >
                <div className="flex flex-col sm:flex-row">
                  <div className="sm:w-48 h-36 sm:h-auto bg-muted relative overflow-hidden flex-shrink-0">
                    <img 
                      src={myCourses[0].image_url || myCourses[0].thumbnail_url || "https://via.placeholder.com/300"} 
              ...
              <section>
                ...
                <div className="grid sm:grid-cols-2 gap-4">
                  {myCourses.map((course) => (
                    <Card key={course.id} className="hover:shadow-md transition-all cursor-pointer overflow-hidden" onClick={() => navigate(`/lesson/${course.id}`)}>
                      <div className="h-32 bg-muted relative">
                        <img src={course.image_url || course.thumbnail_url} alt={course.title} className="w-full h-full object-cover" />
                      </div>
                      <div className="p-4">
      ...
      {/* Bottom Navigation - Mobile Only with 3D Icons */}
      {!isTeacher && (
