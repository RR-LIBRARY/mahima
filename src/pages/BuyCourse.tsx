...
const BuyCourse = () => {
  ...
  // 1. Init Data (Load User & Course) + Auto-enroll for FREE courses + Admin bypass
  useEffect(() => {
    const initData = async () => {
      ...
      if (courseId) {
        ...
        if (!error && data) {
          ...
          // FREE COURSE AUTO-ENROLLMENT (₹0 = skip payment entirely)
          if (data.price === 0 && session?.user) {
            await handleFreeEnrollment(session.user.id, Number(courseId));
          }
        }
      }
      setLoading(false);
  ...
  // 1b. Admin Auto-Enroll: If admin lands on payment page, bypass payment entirely
  useEffect(() => {
  ...
  const handleFreeEnrollment = async (userId: string, courseIdNum: number) => {
    try {
      ...
      // Redirect to the course lessons
      navigate(`/classes/${courseIdNum}/lessons`);
    } catch (error: any) {
  ...
  // 2. Auto Redirect Logic (After payment verification)
  useEffect(() => {
    ...
    if (step === "verify") {
      ...
      timer = setInterval(() => {
        setCountdown((prev) => {
          if (prev <= 1) {
            clearInterval(timer);
            // Redirect to the course lessons after payment
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-10">
      ...
      <main className="max-w-xl mx-auto p-4 mt-4">
        ...
        {/* Step 1: Details - Show FREE enrollment or Payment flow */}
        {step === "details" && (
            <Card>
                <CardHeader>
