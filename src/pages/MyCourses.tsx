...
import { 
  ...
  Filter, GraduationCap, ChevronRight, Loader2
} from "lucide-react";
...
const CourseCard = memo(({ course, onNavigate, formatDate }: { course: EnrolledCourse; onNavigate: (id: number) => void; formatDate: (d: string) => string }) => (
  ...
  >
    <div className="relative h-40 bg-muted overflow-hidden">
      <img
        src={course.thumbnail_url || course.image_url || '/placeholder.svg'}
        alt={course.title}
    ...
    const fetchEnrolledCourses = async () => {
      ...
      try {
        const [enrollmentRes, progressRes] = await Promise.all([
          supabase
            .from('enrollments')
            .select(`purchased_at, course_id, courses (id, title, description, grade, image_url, thumbnail_url, price)`)
        ...
        // Get total lesson counts
        const courseIds = (enrollmentRes.data || []).map((e: any) => e.course_id);
        ...
        const enrolledCourses: EnrolledCourse[] = (enrollmentRes.data || [])
          .filter((item: any) => item.courses)
          .map((item: any) => {
            const total = totalMap[item.courses.id] || 0;
            const completed = completedMap[item.courses.id] || 0;
            return {
              ...item.courses,
