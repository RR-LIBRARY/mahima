...
import { ArrowLeft, Search, ClipboardCheck, Filter, Clock, Loader2 } from "lucide-react";
  ...
  useEffect(() => {
    const fetchTests = async () => {
      ...
      try {
        let query = supabase
          .from("lessons")
          .select("id, title, description, video_url, course_id, category, created_at, position")
          ...
          <div className="space-y-3">
            {filteredTests.map((test) => (
              <div
                key={test.id}
                onClick={() => {
                  if (test.video_url) window.open(test.video_url, "_blank");
                }}
