 ...
 * 
 * Features:
 * 1. Full "Mahima Academy" watermark covering YouTube logo (bottom-right)
...
const MahimaVideoPlayer: React.FC<MahimaVideoPlayerProps> = ({ videoUrl, onEnded, onReady, onDurationReady }) => {
  ...
  const getShortId = (): string => {
    return user?.id ? user.id.slice(-6).toUpperCase() : '';
  };
  ...
  return (
    ...
    >
      ...
      <div className="aspect-video relative">
        ...
        {/* Big play button when paused - ATTRACTIVE PURPLE GLOW */}
        {!playing && isReady && !hasError && (
          ...
          >
            <div 
              className="w-20 h-20 rounded-full flex items-center justify-center transform hover:scale-110 transition-transform duration-300"
              style={{
                background: '#8C4BFF',
                boxShadow: '0 0 30px rgba(140, 75, 255, 0.6), 0 0 60px rgba(140, 75, 255, 0.3)',
              }}
            >
        ...
        <div 
          ...
          aria-hidden="true"
        >
          {/* Invisible tiled pattern for anti-piracy (very subtle - user ID only) */}
          {shortId && (
            <div className="absolute inset-0 grid grid-cols-3 grid-rows-3 gap-4 p-12">
              {[...Array(9)].map((_, i) => (
                <div
                  key={`tile-${i}`}
                  className="flex items-center justify-center text-white/[0.02] text-xs font-medium whitespace-nowrap transform -rotate-45 select-none"
      ...
      {/* Saffron accent bar */}
      <div 
        className="absolute bottom-0 left-0 right-0 h-1 opacity-80 z-50"
