...
import { Play, Pause, Settings, Volume2, VolumeX, Maximize, Minimize, SkipBack, SkipForward, Loader2 } from 'lucide-react';
...
const PremiumVideoPlayer: React.FC<PremiumVideoPlayerProps> = ({ videoUrl, onEnded, onReady }) => {
  ...
  return (
    ...
    >
      ...
      <div className="aspect-video relative" onClick={(e) => e.stopPropagation()}>
        ...
        {/* Clickable overlay DISABLED - click-to-pause is OFF per requirements */}
        {/* Users must use Play/Pause button or Space bar only */}
        ...
        {!playing && isReady && !hasError && (
          ...
          >
            <div className="w-20 h-20 rounded-full bg-black/60 backdrop-blur-sm flex items-center justify-center border-2 border-white/30 transition-transform hover:scale-110">
              <Play className="w-10 h-10 text-white ml-1" fill="white" />
            </div>
          </div>
        )}

        {/* Ghost mask overlay - bottom gradient only, no top gradient */}
        <div className="absolute bottom-0 right-0 w-40 h-16 bg-gradient-to-tl from-black via-black/80 to-transparent z-30 pointer-events-none" />
      </div>
      ...
      <div 
        className={cn(
          "absolute top-4 left-4 text-white/20 text-xs font-semibold pointer-events-none z-50 select-none transition-opacity duration-300",
